...
u8 imuAddr = 0x68<<1;            //IMU sensor address (left shifted by 1 to be 0xD0)

//accelerometer reg addresses
u8 ACCEL_XOUT_H = 0x3B;
u8 ACCEL_XOUT_L = 0x3C;
u8 ACCEL_YOUT_H = 0x3D;
u8 ACCEL_YOUT_L = 0x3E;
u8 ACCEL_ZOUT_H = 0x3F;
u8 ACCEL_ZOUT_L = 0x40;
...
//variables to store accel and gyro data
int16_t accel_x;
int16_t accel_y;
int16_t accel_z;
int16_t gyro_x;
...
void imu_init(){
    ...
    i2c_applyConfig(I2C_CTRL_1, &i2c);                            // Apply the configs from i2c structure into the I2C controller.
    i2c_applyConfig(I2C_CTRL_2, &i2c);
}
...
//variable for last recorded time
float last_timestamp_ms=0;


//IMU sensor configuration function (needed)
void mpu6050_config(u32 reg) {
    u8 val;

    // Wake up MPU6050
    val = 0x00;
    i2c_writeData_b(reg, imuAddr, 0x6B, &val, 1);

    // Set sample rate to 70 Hz: SMPLRT_DIV = 1000 / (1 + val) → val = 13
    val = 13;
    i2c_writeData_b(reg, imuAddr, 0x19, &val, 1);

    // Set DLPF to ~44 Hz (Accel and Gyro bandwidth)
    val = 0x03;
    i2c_writeData_b(reg, imuAddr, 0x1A, &val, 1);

    // Set gyro range to ±250°/s
    val = 0x00;
    i2c_writeData_b(reg, imuAddr, 0x1B, &val, 1);

    // Set accel range to ±2g
    val = 0x00;
    i2c_writeData_b(reg, imuAddr, 0x1C, &val, 1);
}

//converts to m/s^2
float mpu6050_rawToMS2(int16_t raw_accel) {
    return (raw_accel / 16384.0f) * 9.80665f;
}

//scale mpu sensor reading
int16_t scale_mpu_to_adxl367(int16_t mpu_val) {
    return (int16_t)( (mpu_val * 256L) / 16384L );  // scale from 16384 LSB/g to 256 LSB/g
}
...
void main() {
	...
	imu_init(); //initialize IMU I2C

	#endif

	//function call to configure  the imu
	mpu6050_config(I2C_CTRL_1);
	mpu6050_config(I2C_CTRL_2);
	//Fall detection algorithm set up
	FallDetector detect_fall;         //structure definition
	FallDetector* d_fall_ptr=&detect_fall;
	init_fall_detector(d_fall_ptr);  //Initialize



	MotionSample motion_sample;
	motion_sample.accel_mag=0;
	motion_sample.pitch=0;
	motion_sample.roll=0;
	motion_sample.yaw=0;
	motion_sample.timestamp_ms=0;



	IMU_Raw raw_accl_values;
	raw_accl_values.acc_x=0;
	raw_accl_values.acc_y=0;
	raw_accl_values.acc_z=0;
	raw_accl_values.gyro_x=0;
	raw_accl_values.gyro_y=0;
	raw_accl_values.gyro_z=0;
	...
	Attitude* angle_ptr =&angles;

	static int initialized = 0;     //for initializing variables for step counts
	u8 who_am_i = 0x00;    //for debugging

	//buzzer testing
	gpio_setOutputEnable(GPIO1, 1);
//	gpio_setOutput(GPIO1, 0);
//	for(int i=0; i<30; i++){
//		gpio_setOutput(GPIO1, 0);
//		bsp_uDelay(100000);
//		gpio_setOutput(GPIO1, 1);
//		bsp_uDelay(100000);
//	}
	gpio_setOutput(GPIO1, 1);


	bsp_printf(" Out: #%d \r\n", gpio_getOutput(GPIO1));
   ...
   //Posture analysis initialization
      bsp_printf("Measuring baseline CVA: Maintain a good posture\r\n");
      bsp_uDelay(1000*10);
      //Temp varibales for capturing the most sig and least sig 8 bits from the IMU sensor
      u8 high;
      u8 low;

      //Setting the baseline CVA angle (good posture angle)
      //accel x
      i2c_readData_b_2(I2C_CTRL_1, imuAddr, ACCEL_XOUT_H, &high);
      i2c_readData_b_2(I2C_CTRL_1, imuAddr, ACCEL_XOUT_L, &low);
      accel_x =(int16_t)((high << 8) | low);                            //concatenation

      //accel y
      i2c_readData_b_2(I2C_CTRL_1, imuAddr, ACCEL_YOUT_H, &high);
      i2c_readData_b_2(I2C_CTRL_1, imuAddr, ACCEL_YOUT_L, &low);
      accel_y =(int16_t)((high << 8) | low);

      //accel z
      i2c_readData_b_2(I2C_CTRL_1, imuAddr, ACCEL_ZOUT_H, &high);
      i2c_readData_b_2(I2C_CTRL_1, imuAddr, ACCEL_ZOUT_L, &low);
      accel_z =(int16_t)((high << 8) | low);

      //Structure to store accelerometer data (g units) for the posture analysis algorithm
      AccelData imu_reads_cva_set;
      imu_reads_cva_set.ax=accel_x/16384.0f;
      imu_reads_cva_set.ay=accel_y/16384.0f;
      imu_reads_cva_set.az=accel_z/16384.0f;

      //tilt
      float forward_tilt=calculate_tilt_angle(imu_reads_cva_set);
      //cva
      float cva_angle=get_cva_from_tilt(forward_tilt);
      //initialize the algorithm with good posture baseline
      posture_init(cva_angle);
   ...
   while(1) {
     ...
     // MicroPrintf("SYSTEM_CLINT_HZ (hex): %x\n\r", SYSTEM_CLINT_HZ);
      ...
      //IMU implementation for step counts
	#ifdef SYSTEM_I2C_2_IO_CTRL
      //initializing variables for step counts
      ...
      who_am_i = 0x00;
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, 0x75, &who_am_i);
      //bsp_printf(" WHO_AM_I = 0x%02X\r\n", who_am_i);  // should print 0x68

      //accel x
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, ACCEL_XOUT_H, &high);
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, ACCEL_XOUT_L, &low);
      accel_x =(int16_t)((high << 8) | low);                            //concatenation

      //accel y
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, ACCEL_YOUT_H, &high);
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, ACCEL_YOUT_L, &low);
      accel_y =(int16_t)((high << 8) | low);

      //accel z
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, ACCEL_ZOUT_H, &high);
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, ACCEL_ZOUT_L, &low);
      accel_z =(int16_t)((high << 8) | low);

       //gyro x
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, GYRO_XOUT_H, &high);
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, GYRO_XOUT_H, &low);
      gyro_x =(int16_t)((high << 8) | low);

      //gyro y
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, GYRO_YOUT_H, &high);
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, GYRO_YOUT_H, &low);
      gyro_y =(int16_t)((high << 8) | low);

      //gyro z
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, GYRO_ZOUT_H, &high);
      i2c_readData_b_2(I2C_CTRL_2, imuAddr, GYRO_ZOUT_H, &low);
      gyro_z =(int16_t)((high << 8) | low);


      //bsp_printf(" X acceleration in m/s^2: %f\r\n", mpu6050_rawToMS2(accel_x));
      //bsp_printf(" Y acceleration in m/s^2: %f\r\n", mpu6050_rawToMS2(accel_y));
      //bsp_printf(" Z acceleration in m/s^2: %f\r\n", mpu6050_rawToMS2(accel_z));
      //bsp_printf(" X angular velocity in deg/s: %f\r\n", gyro_x/131.0f);
      //bsp_printf(" Y angular velocity in deg/s: %f\r\n", gyro_y/131.0f);
      //bsp_printf(" Z angular velocity in deg/s: %f\r\n", gyro_z/131.0f);

       //Step counting algorithm
       step_counts=StepAlgorithm(accel_x, accel_y, accel_z,step_counts);
       bsp_printf(" Number of steps recorded: #%d \r\n", step_counts);

	#endif


     //IMU implementation for posture analysis and fall detection
   	#ifdef SYSTEM_I2C_1_IO_CTRL

       	 who_am_i = 0x00;
         i2c_readData_b_2(I2C_CTRL_1, imuAddr, 0x75, &who_am_i);
        // bsp_printf(" WHO_AM_I = 0x%02X\r\n", who_am_i);  // should print 0x68
         ...
         //gyro x
         i2c_readData_b_2(I2C_CTRL_1, imuAddr, GYRO_XOUT_H, &high);
         i2c_readData_b_2(I2C_CTRL_1, imuAddr, GYRO_XOUT_H, &low);
         gyro_x =(int16_t)((high << 8) | low);

         //gyro y
         i2c_readData_b_2(I2C_CTRL_1, imuAddr, GYRO_YOUT_H, &high);
         i2c_readData_b_2(I2C_CTRL_1, imuAddr, GYRO_YOUT_H, &low);
         gyro_y =(int16_t)((high << 8) | low);

         //gyro z
         i2c_readData_b_2(I2C_CTRL_1, imuAddr, GYRO_ZOUT_H, &high);
         i2c_readData_b_2(I2C_CTRL_1, imuAddr, GYRO_ZOUT_H, &low);
         gyro_z =(int16_t)((high << 8) | low);
         ...
         //fall detection algorithm (IMU)

         //updating raw values
         raw_accl_values.acc_x=accel_x;
         raw_accl_values.acc_y=accel_y;
         raw_accl_values.acc_z=accel_z;
         raw_accl_values.gyro_x=gyro_x;
         raw_accl_values.gyro_y=gyro_y;
         raw_accl_values.gyro_z=gyro_z;

         uint32_t curr_time=get_time_ms();
         float dt = (curr_time- last_timestamp_ms) / 1000.0f;      // in seconds
         compute_attitude(raw_accl_values, angle_ptr, dt);         // update the angles
         last_timestamp_ms=curr_time;								// store current time

         //update motion sample
         motion_sample.accel_mag=compute_accel_mag(accel_x,accel_y,accel_z);
         motion_sample.pitch=angles.pitch;
         motion_sample.roll=angles.roll;
         motion_sample.yaw=angles.yaw;
         motion_sample.timestamp_ms=get_time_ms();

   //      bsp_printf("Current magnitude: %f\r\n", motion_sample.accel_mag);
   //      bsp_printf("Pitch: %f\r\n",  motion_sample.pitch);
   //      bsp_printf("Roll: %f\r\n",  motion_sample.roll);
   //      bsp_printf("Yaw: %f\r\n",  motion_sample.yaw);

         update_fall_detector(d_fall_ptr, motion_sample);   // detect for any falls

         if(detect_fall.alert_triggered==true){     //fall is confirmed
       	  bsp_printf(" User fell \r\n");
       	  bsp_printf("Fall type %d\r\n", detect_fall.detected_fall);
       	  reset_fall_detector(d_fall_ptr);                         //reset

         }
         else{
       	 bsp_printf(" No falls detected \r\n");
         }
         ...
         imu_reads_cva_set.ay=accel_y/16384.0f;
         imu_reads_cva_set.az=accel_z/16384.0f;
         update_posture_state(imu_reads_cva_set);

         PostureCase curr_pos=get_posture_case();
         if(curr_pos==0){
       	  //bsp_printf("You are currently in a good posture \r\n");
       	  gpio_setOutput(GPIO1, 0);
         }
         else{
       	  //bsp_printf("You are currently in a bad posture \r\n");
       	  gpio_setOutput(GPIO1, 1);
         }
